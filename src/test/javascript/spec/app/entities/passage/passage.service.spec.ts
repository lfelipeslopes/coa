/* tslint:disable max-line-length */
import { TestBed, getTestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { HttpClient, HttpResponse } from '@angular/common/http';
import { of } from 'rxjs';
import { take, map } from 'rxjs/operators';
import * as moment from 'moment';
import { DATE_TIME_FORMAT } from 'app/shared/constants/input.constants';
import { PassageService } from 'app/entities/passage/passage.service';
import { IPassage, Passage } from 'app/shared/model/passage.model';

describe('Service Tests', () => {
    describe('Passage Service', () => {
        let injector: TestBed;
        let service: PassageService;
        let httpMock: HttpTestingController;
        let elemDefault: IPassage;
        let currentDate: moment.Moment;
        beforeEach(() => {
            TestBed.configureTestingModule({
                imports: [HttpClientTestingModule]
            });
            injector = getTestBed();
            service = injector.get(PassageService);
            httpMock = injector.get(HttpTestingController);
            currentDate = moment();

            elemDefault = new Passage(0, 0, false, 0, currentDate, 0, 'AAAAAAA', currentDate);
        });

        describe('Service methods', async () => {
            it('should find an element', async () => {
                const returnedFromService = Object.assign(
                    {
                        occurrenceDate: currentDate.format(DATE_TIME_FORMAT),
                        processedAt: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                service
                    .find(123)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: elemDefault }));

                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should create a Passage', async () => {
                const returnedFromService = Object.assign(
                    {
                        id: 0,
                        occurrenceDate: currentDate.format(DATE_TIME_FORMAT),
                        processedAt: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        occurrenceDate: currentDate,
                        processedAt: currentDate
                    },
                    returnedFromService
                );
                service
                    .create(new Passage(null))
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'POST' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should update a Passage', async () => {
                const returnedFromService = Object.assign(
                    {
                        amount: 1,
                        automaticPassage: true,
                        lane: 1,
                        occurrenceDate: currentDate.format(DATE_TIME_FORMAT),
                        passageIdentification: 1,
                        plate: 'BBBBBB',
                        processedAt: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );

                const expected = Object.assign(
                    {
                        occurrenceDate: currentDate,
                        processedAt: currentDate
                    },
                    returnedFromService
                );
                service
                    .update(expected)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'PUT' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should return a list of Passage', async () => {
                const returnedFromService = Object.assign(
                    {
                        amount: 1,
                        automaticPassage: true,
                        lane: 1,
                        occurrenceDate: currentDate.format(DATE_TIME_FORMAT),
                        passageIdentification: 1,
                        plate: 'BBBBBB',
                        processedAt: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        occurrenceDate: currentDate,
                        processedAt: currentDate
                    },
                    returnedFromService
                );
                service
                    .query(expected)
                    .pipe(
                        take(1),
                        map(resp => resp.body)
                    )
                    .subscribe(body => expect(body).toContainEqual(expected));
                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify([returnedFromService]));
                httpMock.verify();
            });

            it('should delete a Passage', async () => {
                const rxPromise = service.delete(123).subscribe(resp => expect(resp.ok));

                const req = httpMock.expectOne({ method: 'DELETE' });
                req.flush({ status: 200 });
            });
        });

        afterEach(() => {
            httpMock.verify();
        });
    });
});
